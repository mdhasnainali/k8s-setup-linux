...
Reverses [`k8s-setup worker install`](worker-install.md) (and any subsequent `kubeadm join`) on a worker node: resets kubeadm, purges kubelet/kubeadm/kubectl and optionally the container runtime, removes their apt repos/keys/config, and restores swap/sysctl/kernel-module changes made during setup — so the node returns to a pre-setup state.
...
k8s-setup worker uninstall --purge-runtime   # also remove the container runtime
k8s-setup worker uninstall -y           # non-interactive: reset, but keep the runtime
