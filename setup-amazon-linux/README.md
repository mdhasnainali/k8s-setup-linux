# Kubernetes Setup Scripts (Amazon Linux)

Scripts to bootstrap a Kubernetes cluster on Amazon Linux 2 / Amazon Linux 2023 nodes.
...
- Amazon Linux 2 or Amazon Linux 2023 host (uses `dnf`; falls back to `amazon-linux-extras` on AL2 for containerd)
- Root/sudo access
- Internet access to `pkgs.k8s.io`, `dl.k8s.io`, GitHub
...
- Registers `/etc/yum.repos.d/kubernetes.repo` pinned to the resolved `v<major.minor>` channel; `dnf` verifies the repo's `gpgkey` itself (no manual key import needed, unlike apt).
- The k8s rpm repo appends its own build revision to versions (e.g. `1.33.0-150500.1.1`), so the script looks up the exact matching package string via `dnf list --showduplicates` rather than guessing it.
- `--disableexcludes=kubernetes` overrides the repo's own `exclude=` line (which exists to stop *other* repos/updates from touching these packages) so this script's own install/lock commands can still target them.
- `dnf versionlock add` pins the installed versions — the rpm/dnf equivalent of `apt-mark hold` — so a routine `dnf update` can't silently bump them; version skew between kubelet/kubeadm/kubectl (or across nodes) breaks clusters.

**Step 2 — Disable swap, load kernel modules, relax SELinux**
...
- If SELinux is `Enforcing` (some Amazon Linux 2 AMIs ship this way; AL2023 defaults to disabled), it's switched to `Permissive` — kubeadm's own getting-started docs assume this unless you've set up the container-selinux policies yourself.
...
- Amazon Linux 2023 ships `containerd` directly in its base repos. Amazon Linux 2 does not — `amazon-linux-extras install docker` pulls it in as a dependency instead. The script detects which OS it's on via `/etc/os-release` and picks the right path.
...
- `--control-plane-endpoint=$(hostname)` uses the hostname rather than a bare IP, keeping the option open to later grow into an HA control-plane behind a stable name.
  ...
  - **Bare IP** (e.g. `--control-plane-endpoint=10.0.1.50`, or an EC2 private IP): fine for a single node. Add more control-plane nodes later for HA and that IP is just one of several — it's not a shared front door. Switching means regenerating certs and reconfiguring every client, with likely downtime.
  - **Hostname** (e.g. `--control-plane-endpoint=k8s-cluster.mycompany.local`): certs/kubeconfigs only ever reference the name, not what's behind it. Today DNS points that name straight at the one node. Add more control-plane nodes later, drop a load balancer (e.g. an internal NLB) in front of them, and just repoint the DNS name at the load balancer — no cert regen, no client reconfig.
...
- `--ignore-preflight-errors=all` skips kubeadm's preflight checks — convenient for VMs/lab boxes (including small EC2 instance types) with non-standard resources, but worth revisiting before production use.
...
- On EC2, make sure the security group allows the ports kubeadm needs between nodes (API server 6443, etcd 2379-2380, kubelet 10250, etc.) — this script does not touch security groups or any firewall.
